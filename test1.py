...
def main(subset_years=None):
    ...
    # Fresh start: drop and rebuild, since the old table has known bad rows
    ...
    failed_files = 0
    start = time.time()
    folder_stats = {}  # top-level folder -> (attempted, inserted)

    for dirpath, dirnames, filenames in os.walk(root_dir):
        if ".git" in dirpath:
            continue
        # top-level folder name, e.g. "2018" or "2018\IntradayData_2018"
        ...
        for fname in data_files:
            ...
            if len(batch) >= 20000:
                ...
                a, i = folder_stats.get(top, (0, 0))
                folder_stats[top] = (a + len(batch), a + (cur.rowcount if cur.rowcount != -1 else 0))
                batch = []
                conn.commit()
    ...
    print("\nPer top-level-folder attempted vs inserted (spot duplicate sources here):")
...
if __name__ == "__main__":
    # TEST FIRST on the messiest years before committing to the full 142k-file run:
    main(subset_years=["2017", "2018"])
    # Once that looks right, comment out the line above and run the full set:
    # main()